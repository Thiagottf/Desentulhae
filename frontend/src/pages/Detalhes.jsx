...
const Detalhes = () => {
  ...
  // Exemplo de preço fictício - se quiser usar de verdade, adicione no PublicarEntulho
  const [price] = useState("R$ 1.900")
  ...
  useEffect(() => {
    ...
    if (foundPost) {
      setPost(foundPost)
      // Verifica se o anúncio já está salvo
  ...
  // Simula envio de notificação de compra para o anunciante
  const handleBuy = () => {
    const loggedUser = JSON.parse(localStorage.getItem("usuarioLogado"))
    if (!loggedUser) {
  ...
  const handleOpenChat = () => {
    ...
    // Redireciona para a página de chat (você pode passar o ID do post via query param)
    navigate("/mensagens")
  }
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-4">
      ...
      <div className="max-w-6xl mx-auto bg-white p-6 rounded-lg shadow">
        {/* Layout em colunas: Imagens à esquerda, infos à direita */}
